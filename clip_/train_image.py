...
def train(model, optimizer, loss_func, sched, metric_fc, train_dl, val_dl, n_epochs, train_df, val_df,
             ...
             prev_best_info={'val': {'thr': None, 'f1': None},
                             'train': {'thr': None, 'f1': None}},
    ...
    clip.model.convert_weights(metric_fc)
    convert_models_to_fp32_(model)
    convert_models_to_fp32_(metric_fc)
    ...
    for ep in tqdm(range(ep_start, ep_start + n_epochs), leave=False):
        ...
        for imgs, labels in pbar:
            ...
            #     if p.grad is None :
            #         print(n)
            # convert_models_to_fp32(model.model)
            # convert_models_to_fp32(metric_fc)
            optimizer.step()
            # clip.model.convert_weights(model.model)
            # clip.model.convert_weights(metric_fc)
            # metric_fc.weight.data = metric_fc.weight.data.half()
            sched.step()
        ...
        if prev_best_info['train']['thr'] is None:
            ...
            thrs = [prev_best_info['train']['thr'] - 0.1, prev_best_info['train']['thr'] - 0.05, prev_best_info['train']
                    ['thr'], prev_best_info['train']['thr'] + 0.05, prev_best_info['train']['thr'] + 0.1]
        ...
        with torch.no_grad():
            ...
            if prev_best_info['val']['thr'] is None:
                ...
                thrs = [prev_best_info['val']['thr'] - 0.1, prev_best_info['val']['thr'] - 0.05, prev_best_info['val']
                        ['thr'], prev_best_info['val']['thr'] + 0.05, prev_best_info['val']['thr'] + 0.1]
            val_f1s, best_thresh_val, f1_val = compute_f1(
                embs, val_df['label_group'], thrs)
...
def freeze_to(model, layer) :
    layers = ['resblock.{}'.format(i) for i in range(0, layer+1)]
    c=0
    for n, p in model.named_parameters() :
        if any(layer in n for layer in layers) :
            c+=1
            p.requires_grad = False
    print('Froze {} params'.format(c))
def unfreeze_to(model, layer) :
    layers = ['resblock.{}'.format(i) for i in range(layer,12)]
    c = 0
    for n, p in model.named_parameters() :
        if any(layer in n for layer in layers) :
            c += 1
            p.requires_grad = True
    print('Unfroze {} params'.format(c))

def freeze(model) :
    for p in model.parameters() :
        p.requires_grad = False
    print('Froze {} params'.format(len(list(model.parameters()))))

def unfreeze(model) :
    for p in model.parameters() :
        p.requires_grad = True
        print('UFroze {} params'.format(len(list(model.parameters()))))
